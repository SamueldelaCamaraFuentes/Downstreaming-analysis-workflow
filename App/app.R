                     ...
                     sidebarMenu(
                       ...
                       menuItem("Preprocessing", icon = icon("th"), tabName = "preprocessing",
                                ...
                                numericInput(inputId = "minfiltro",
                                             label = "Minimum values for filtering",
                                             value = 0),
                                h3("Normalization"),
                       ...
                       menuItem("Download tables", icon = icon("download"), tabName = "Download",
                                textInput(inputId = "filenamedownload",
                                          label = "Filename",
                                          value = "data"),
                                downloadButton(outputId = "downloaddf", icon("download"),
                                               label = "Download",
                                               style="display: block; margin: 0 auto; width: 200px; color:black;"))
                     )),
                     ...
                     sidebarMenu(
                       menuItem( "Quality metrics", icon = icon("chart-line"),
                                 ...
                                 actionButton( inputId = "box",
                                               label = "Render plot"),
                                 strong(h4("Dispersion plot")),
                                 actionButton( inputId = "cv2",
                                               label = "Render plot"),
                                 strong(h4("Pre imputation state plot")),
                                 actionButton( inputId = "preimputation",
                                               label = "Render plot"),
                                 strong(h4("Post imputation state plot")),
                                 actionButton( inputId = "postimputation",
                                               label = "Render plot"),
                                 strong(h4("Histogram")),
                ...
                tabPanel(title = "Quality metrics",
                         ...
                         fluidPage(
                           fluidRow(
                             ...
                             box(
                               title = "Dispersion plot", width = 6, status = "primary",
                               plotOutput(outputId = "cv2plot", height = "600px")
                             ),
                             box(
                               title = "Pre imputation plot", width = 6, status = "primary",
                               plotOutput(outputId = "preimputationplot", height = "600px")
                             ),
                             box(
                               title = "Post imputation plot", width = 6, status = "primary",
                               plotOutput(outputId ="postimputationplot", height = "600px")
                             ),
                             box(
                         ...
                         fluidPage(
                           fluidRow(
                             box(
                               ...
                               plotOutput(outputId = "volcanoplot", height = "600px")
                             ),
                             box(
...
server <- function(input, output) {
  ...
  #CV2 plot
  output$cv2plot <- renderPlot({
    if (input$cv2 == 0){
      return(NULL)
    } else if (input$cv2 > 0){
      plotCV2(data()[,LOG2.names()],  trend = TRUE, main = "Dispersion check", cex = 0.2, pch = 16, xlab="Average log-intensity", ylab=expression("Relative standard deviation"))}
  ...
  output$postimputationplot <- renderPlot({
    if (input$postimputation == 0){
      return(NULL)
    } else if (input$postimputation > 0){
  ...
  difexpression <- reactive({
    validate(need(!is.null(input$pvaladj),
                  "Please select an option"))
    limma <- limma.analysis(data(), LOG2.names(), input$testid, input$repcond1, input$repcond2, input$condition1, input$condition2, input$orderdif, input$LogFCup, input$LogFCdown, input$sigcutoff, input$pvaladj)
    return(limma)
  ...
  output$limma <- DT::renderDataTable({
    ...
    datatable(difexpression(), options = list(pageLength = 5,
                                              lengthMenu = c(5, 10, 15, 20), 
                                              scrollX = T,
                                              autoWidth = TRUE ))
  })
  ...
  #Volcano
  volcano <- reactive({
    volcano <- volcano_plot(difexpression(), input$volcanotitle, input$labelprots)
    return(volcano)
  })
  ...
  funcanalysis <- reactive({
    ...
    Go_terms <- Goterms_finder(difexpression(), target = input$proteinid, numeric_ns = "", mthreshold = Inf, filter_na = TRUE, organismo = input$organism, user_threshold = input$userthreshold, multi_query = FALSE, evcodes = TRUE, sources = c("GO", "KEGG", "WP", "REAC", "CORUM"))
    return(Go_terms)  
  })
  ...
  interactions <- reactive({
     interactions <- interactions_up(difexpression())
    return(interactions)
  ...
  output$downreg <- renderPlot({
    if (input$downnet == 0){
      ...
      interactions_down(difexpression())
    }
  }
  ...
  output$downloaddownnetwork <- downloadHandler(
    ...
    content = function(file) {
      if (input$funcextension == "tiff"){
        tiff(file)
        interactions_downp(difexpression())
        dev.off()
      } else if (input$funcextension == "pdf") {
        pdf(file)
        interactions_down(difexpression())
  ...
  graph <- reactive({
    graph_analysis <- igraph_analysis(interactions())
