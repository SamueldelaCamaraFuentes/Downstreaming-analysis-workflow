...
##################################################################################
#Limma function
statistical_analysis <- function(df, LOG2.names,test, paired = FALSE, replicas_condicion1, replicas_condicion2, condition1, condition2, orden, logfcup, logfcdown, sig, adjval){
  if (test == 2){
  ...
  p.mod <- fit.eb$p.value[, 2]    # p-valor moderado correspondiente al estad?stico t moderado.
  ...
  return(results.eb)
    ...
    colnames(results.eb) <- c("logFC", "p.mod", "tStat")
    results.eb$expression <- case_when(results.eb$logFC >= logfcup & -log10(results.eb$p.mod) >= sig ~ "Up-regulated",
                                       results.eb$logFC <= logfcdown & -log10(results.eb$p.mod) >= sig ~ "Down-regulated",
                                          TRUE ~ "Unchanged")#labels para expresion 
    
    results.eb$adj.P.Val <- p.adjust(results.eb$p.mod, method = adjval)
    if (orden == 1){
      results.eb <- results.eb[order(results.eb$p.mod), ] #Ordenamos por p.mod
    } else if (orden ==2){
      results.eb <- results.eb[order(results.eb$adj.P.Val), ]
...
t_test <- function(df,LOG2.names, paired = FALSE, condition1, condition2, orden, logfcup, logfcdown, sig, adjval){
  ...
  colnames(proteinGroups) <- c("logFC", "p.mod", "tStat")
  proteinGroups$expression <- case_when(proteinGroups$logFC >= logfcup & -log10(proteinGroups$p.mod) >= sig ~ "Up-regulated",
                                        proteinGroups$logFC <= logfcdown & -log10(proteinGroups$p.mod) >= sig ~ "Down-regulated",
                                        TRUE ~ "Unchanged")#labels para expresion 
  
  proteinGroups$adj.P.Val <- p.adjust(proteinGroups$p.mod, method = adjval)
  if (orden == 1){
    proteinGroups <- proteinGroups[order(proteinGroups$p.mod), ] #Ordenamos por p.mod
  } else if (orden ==2){
